"""Regression tests for TASK-048 / REQUIREMENTS_BUTLER_PULL.md.
...
this repo's own Makefile, templates/, and claude-agents/, the same content
`git subtree add --prefix=.butler` vendors into a consumer project) and a
"consumer" repo that has already adopted it and trimmed once, as a real
project would after following the README's adoption steps.
...
def _build_upstream(path: Path) -> None:
    """A fixture butler source repo: this repo's real Makefile + templates/ + claude-agents/."""
    _init_repo(path)
...
class TestButlerPullTrimsAutomaticallyWhenTemplatesUnchanged:
    """Scenario: a pull that did not touch .butler/templates/ or
    .butler/claude-agents/ behaves exactly as it did before TASK-048 —
    fetch and trim in one step, no pause."""
    ...
    def test_trim_runs_automatically_and_no_warning_is_printed(self, tmp_path: Path) -> None:
        ...
        assert not (consumer / ".butler" / "templates").exists(), (
            "butler-pull must still auto-trim when templates/claude-agents did not change"
        )
